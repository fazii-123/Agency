...
class DashboardStatsView(views.APIView):
    ...
    def get(self, request):
        try:
            ...
            service_distribution = ContactSubmission.objects.values('service').annotate(count=Count('service'))
            service_map = dict(ContactSubmission._meta.get_field('service').choices)
            
            service_data = [
                {'name': service_map.get(item['service'], item['service']), 'value': item['count']}
            ...
            if previous_month_count > 0:
                percentage_change = ((last_month_submissions - previous_month_count) / previous_month_count) * 100
            else:
            ...
            # Status counts
            new_count = ContactSubmission.objects.filter(
                Q(status='new') | Q(status__isnull=True)
            ).count()
